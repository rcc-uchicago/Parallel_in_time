...
class FineTask(object):
    ...
    def prepare_input(self, c2f_bin = None, f2c_bin = None, adv_bin = None):
        ...
        if (self.iter == 1 or self.first_in_slice):
            fine_in = 'fine_in.' + self.suffix
            print 'coarse to fine in first invocation of fine for the current slice'
            if (self.slice == 1):
                shutil.copyfile(coarse_out_prv_slice['COARSE_OUT'], fine_in)
            else:
                call_list = [c2f_bin, coarse_out_prv_slice['COARSE_OUT'], fine_in, '1', '192', 
                                                       '192', '100', '100']
            ...
            if converge_prev_slice:
                ...
                fine_in = fine_out_prev_slice['FINE_OUT']
            else:
                fine_in = coarse_out_prv_slice['COARSE_ADVANCE_PADDED']
            
        out_file = 'fine_out.' + self.suffix
        energy_file = 'energy_fine.' +  self.suffix
        args = self.cmd_args + [fine_in, out_file, energy_file, self.suffix]
        self.out_files = {'FINE_OUT' : os.path.join(self.working_dir, out_file),
                          'ENERGY_OUT' : os.path.join(self.working_dir, energy_file)}
...
class Fine(Component):
    def __init__(self, services, config):
        ...
        self.cmd_args = self.FINE_PARAMS.split()
    ...
    def init(self, timeStamp = 0.0):
        ...
        coarse_out_1_0 = self.INPUT_FILES.split()[1]        
                
        task_11 = FineTask(self, iteration, slice, 
                           working_dir = self.working_dir,
                           cmd_args = self.cmd_args)
        
        coarse_out_dict = {'COARSE_OUT' : os.path.join(self.working_dir, coarse_out_1_0),
