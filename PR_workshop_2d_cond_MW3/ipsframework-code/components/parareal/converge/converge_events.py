...
class ConvergeTask(object):
    ...
    def prepare_input(self):
        ...
        if (fine_in_cur_iter and fine_in_prev_iter):
            prev_iter_energy = fine_in_prev_iter['ENERGY_OUT']
            cur_iter_energy = fine_in_cur_iter['ENERGY_OUT']
            converge_out = 'converge_out.' + self.suffix
            self.submit_args = [prev_iter_energy, cur_iter_energy, converge_out]
        ...
        while not done:
            if len(self.ready_tasks) > 0:
                ...
                if not args:
                    ...
                    print 'Converge Command ', cmd_list 
                    subprocess.call(cmd_list)
                    conv_file = new_task.get_external_outputs()['CONVERGE_OUT'] 
                    f_conv_file = open(conv_file, 'r')
                    conv_str = f_conv_file.readline().strip()
                    if conv_str == '1':
                        slice_converged = True
                    number_str = f_conv_file.readline().split()
                    err = float(number_str[0])
                    energy = float(number_str[1])
                    err_str = '%10.3e' % err
                    energy_str = '%12.6e' % energy
                    error[task_iter, task_slice] = err
                    f_conv_file.close()
                    print 'convergence string = ', conv_str, 'iteration = ', new_task.iter
