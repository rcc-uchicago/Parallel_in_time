...
!
  ...
  character(len=128) c_time_final,c_time_init, c_pe_slice, c_time_step_num !Variable for time setup for slice in PR-SD
      ...
      character(len = 1680) :: file_values_old, file_values_new
      character(len = 1680) :: file_conv_result !all variables for reading in  file names  -SD
      character(len = 20) :: cnr,err_tol !number of rows, read as character
      integer :: nr
      double precision,dimension(:),allocatable:: values_new,values_old
      double precision :: err_en_av, err_tolerance
...
!Command line entries for PR-SD
 n_args = command_argument_count() !number of arguments from command line for parareal PYTHON framework -SD
      call get_command_argument(1, file_values_old)
      call get_command_argument(2, file_values_new)
      call get_command_argument(3, file_conv_result)
      call get_command_argument(4, cnr)
      call get_command_argument(5, err_tol)

      !Convert characters to integers- SD
      read(cnr,'(i10)') nr
      read(err_tol,'(f24.20)') err_tolerance

      allocate(values_new(nr),values_old(nr))
      values_new=0.
      values_old=0.

      open(141,file= trim(file_values_old),status='unknown')
      open(142,file= trim(file_values_new),status='unknown')
      open(143,file= trim(file_conv_result),status='unknown')

      read(141,'(g14.6)',end=300)(values_old(i),i=1,nr)
300  write(6,*) "number of lines in values_old", i-1  

     read(142,'(g14.6)',end=301)(values_new(i),i=1,nr)
301  write(6,*) "number of lines in values_new", i-1     
...
!
  ...
  do time_step = 1, 1!time_step_num

    u=values_new
    u_exact = values_old
    call errors ( element_area, element_node, node_xy, u, &
     u_exact, element_num, element_order, node_num, time, el2, eh1)!Modified for PR-SD

    !write ( 143, '(5g14.6)' ) time, el2, eh1, u(500), u(3000)
    err_en_av=eh1
     if (err_en_av <= err_tolerance) then
       write(143,*) 1,err_en_av !=1 if convergence
     else
       write(143,*) 0,err_en_av !=0 if convergence
     end if
     write(6,*) "ERR_ACTUAL IS ", err_en_av
...
subroutine errors ( element_area, element_node, node_xy, u, &
 u_exact, element_num, element_order, node_num, time, el2, eh1 ) 
...
!
  ...
  real ( kind = 8 ), dimension(node_num) :: u_exact 
  real ( kind = 8 ) uh,uex
...
!
    do quad = 1, quad2_num
      ...
      do in1 = 1, element_order
        ...
        uex    = uex    + bi    * u_exact(i)
        duexdxh = duexdxh + dbidx * u_exact(i)
        duexdyh = duexdyh + dbidy * u_exact(i)
