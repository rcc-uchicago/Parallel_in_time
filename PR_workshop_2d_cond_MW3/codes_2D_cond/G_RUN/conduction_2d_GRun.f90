...
!
  ...
  character(len = 1680) :: file_in_val !Variable for filename for reading in u -SD
  character(len = 1680) :: result_out, restart_val !Variable for filenames for writing out u & errors-SD
  integer*8 :: pe_slice
...
!Command line entries for PR-SD
 ...
 call get_command_argument(1, file_in_val)
 call get_command_argument(2, restart_val)
 call get_command_argument(3, result_out)
 call get_command_argument(4, c_time_init)
 call get_command_argument(5, c_time_final)
 call get_command_argument(6, c_pe_slice)
 call get_command_argument(7, c_time_step_num)
...
!
  ...
  !call nodes_write ( node_num, node_xy, node_txt_file_name ) ! Modified for PR-SD
...
!
  ...
  !call element_write ( element_order, element_num, element_node, &
   ! triangulation_txt_file_name )! Modified for PR-SD
...
!
  !time_init = 0.0D+00 ! Modified for PR-SD
  !time_final = 0.2D+00 ! Modified for PR-SD
  !time_step_num = 2 ! Modified for PR-SD

  !convert to integers & reals for PR-SD 
  read(c_time_step_num,'(i8)') time_step_num  
  read(c_time_init,'(g14.6)') time_init_tmp !Time read in, typically 0.2
  read(c_time_final,'(g14.6)') time_final_tmp !Time read in, typically 1.0
  read(c_pe_slice,'(i8)') pe_slice !typically slice number

  write(6,*) "slice is", pe_slice, "for input", c_pe_slice

  !Actually, calculate times based on slice in PR-SD
  time_init=(real(pe_slice, kind=8)-1.0)*(time_final_tmp-time_init_tmp) + time_init_tmp
  time_final= time_final_tmp + (real(pe_slice, kind=8)-1.0)*(time_final_tmp-time_init_tmp)
...
!
  ...
  !u(1:node_num) = u_exact(1:node_num) !Modified for PR-SD
!Values read in as input -SD
  open(50,file=trim(file_in_val),status='unknown')  
  read(50,'(g14.6)') (u(ival),ival=1,node_num)
  close(50)

  !call get_unit ( time_unit ) ! Modified for PR-SD
  !open ( unit = time_unit, file = time_file_name, status = 'replace' ) ! Modified for PR-SD
  !write ( time_unit, '(g14.6)' ) time ! Modified for PR-SD
  !call solution_write ( node_num, u, u_file_name ) ! Modified for PR-SD
!
!  Time looping.
!
  write ( *, '(a)' ) ' '
 ...
 open(71,file=result_out,status='unknown') ! Introduced for PR-SD
...
!      element_num, element_order, node_num, time, el2, eh1)!Modified for PR-SD

    write ( 71, '(5g14.6)' ) time, el2, eh1, u(500), u(3000)
...
!    call solution_write ( node_num, u, u_file_name )
  ...
  !close ( unit = time_unit )! Modified for PR-SD

   open(70,file=restart_val,status='unknown')
    do node = 1, node_num
    write (70,'(g14.6)') u(node)
    end do
   close(70)
   close(71)
!  Deallocate memory.
